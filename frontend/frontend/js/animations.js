...
/* ==============================================
   LÓGICA DEL CARRUSEL (MODO SEGURO - SIN TOCAR CSS)
   ============================================== */
document.addEventListener('DOMContentLoaded', () => {
    // Usamos los nombres exactos de tu index.html
    const contenedor = document.querySelector('.carrusel-container');
    const diapositivas = document.querySelectorAll('.carrusel-item');

    // Si no existen los elementos, salimos para no generar errores en consola
    if (!contenedor || diapositivas.length === 0) return;

    let indiceActual = 0;
    const duracion = 5000; 

    // --- CONFIGURACIÓN DINÁMICA ---
    // En lugar de editar el archivo .css, aplicamos lo mínimo necesario por JS
    // para asegurar que las imágenes estén en fila y se muevan.
    contenedor.style.display = 'flex';
    contenedor.style.transition = 'transform 0.7s ease-in-out';
    
    diapositivas.forEach(slide => {
        slide.style.minWidth = '100%'; // Asegura que cada foto ocupe todo el ancho
    });
    ...
    function moverSiguiente() {
        ...
        // Aplicamos el movimiento mediante transform
        contenedor.style.transform = `translateX(${desplazamiento}%)`;
    }

    // Guardamos el intervalo para que sea estable
    setInterval(moverSiguiente, duracion);
});